...
- `GET /api/data-health`：实际调用东方财富直连接口，失败后依次回退 Yahoo Finance 和 AkShare 单股票接口，检查真实财经数据源连通性，并返回 `provider_checks` 诊断。

## 说明

- 首版优先支持 A 股。
- 财务报表优先使用东方财富公开接口，失败后回退 AkShare。
- 模型会自动补齐核心输入：FCF 缺失时依次用经营现金流、净利润、收入 10% 代理；现金/债务缺失时按 0 暂估；股本缺失时依次用行情股本、市值/股价推导。所有代理项都会写入警告，关键估值请用覆盖项复核。
- 东方财富、Yahoo Finance 和 AkShare 都不可用时，后端会返回错误；线上版本不会用演示数据冒充真实数据。
